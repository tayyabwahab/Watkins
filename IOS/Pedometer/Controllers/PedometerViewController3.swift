...
class PedometerViewController3: UIViewController {
    ...
    private var forwardGifFemale = "02_Women_moving_forward"
    ...
    private var distanceTravel = 0.0 {
        didSet {
            ...
            if distanceTravel > 1 && currentGif == forwardGifFemale {
...
//            }
            ...
            if  distanceTravel >= Double(completeDistance) && currentGif == forwardGifFemale {
                currentGif = ""
...
//                    self.stepCounter = Double(self.runCount) + self.stepCounter
                    ...
                    if self.stepCounter == 12.0 && self.currentsituation == "stationary"{
                      //alert message to quit the session if no movement
                        self.alert3(message: "Did not read any Movement data, make sure to keep walking! ")
    ...
    private func getResultControllerInstance() -> ResultViewController3? {
        if let reultVC = self.storyboard?.instantiateViewController(withIdentifier: "ResultViewController3") as? ResultViewController3 {
            return reultVC
        }
        return nil
    }
    ...
    @IBAction func popViewController(_ sender: Any) {
        ...
        self.navigationController?.backToViewController(viewController: StartViewController3.self)
    }
}
