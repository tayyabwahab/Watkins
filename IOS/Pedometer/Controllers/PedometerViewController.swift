...
class PedometerViewController: UIViewController {
    ...
    private var distanceTravel = 0.0 {
        didSet {
            ...
            if distanceTravel > 1 && currentGif == standupGif {
            ...
            if distanceTravel >= (Double(completeDistance) * 0.90) && distanceTravel < Double(completeDistance) && currentGif == backwardGif {
                playSound(music: sitDown)
                currentGif = sitdownGif
                updateGif(gifName: currentGif)
            }
            
            if  distanceTravel >= Double(completeDistance) && currentGif == sitdownGif {
                currentGif = ""
...
//                    self.stepCounter = Double(self.runCount) + self.stepCounter
                    ...
                    if self.stepCounter == 12.0 && self.currentsituation == "stationary"{
                      //alert message to quit the session if no movement
                        self.alert(message: "Did not read any Movement data, make sure to keep walking! ")
    ...
    private func getResultControllerInstance() -> ResultViewController? {
        if let reultVC = self.storyboard?.instantiateViewController(withIdentifier: "ResultViewController") as? ResultViewController {
            return reultVC
        }
        return nil
    }
    ...
    private func showResultsVC() {
        ...
        DispatchQueue.main.async { [weak self] in
            if let resultVC = self?.getResultControllerInstance() {
                ...
                resultVC.distance = String(format: "%.1f", self?.distanceTravel ?? 0)
